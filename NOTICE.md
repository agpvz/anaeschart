...
- the interactive vitals chart — plotting, symbols, derived mean arterial pressure,
  the snapped time axis and the selectable column interval
- the observation-table import (`fromRows()`, `parseTable()`, `importAny()`)
- the self-contained HTML export, the print pipeline and the headless PDF tooling

No licence is granted. All rights reserved.

## Form content — © South African Society of Anaesthesiologists (SASA), v2020.

The content of the form — its layout, structure, field arrangement, bilingual clause
wording and text — is SASA's copyright. The SASA name and logo are SASA's marks.

This is an unofficial implementation. It is not published, endorsed, certified,
approved by, or affiliated with SASA. The SASA mark drawn in `index.html` is an
approximation standing in for the official asset; replace it with the official asset
or remove it before any distribution.
